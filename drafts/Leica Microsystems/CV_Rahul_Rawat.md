...
Data Science and Analytics Master student at SRH Heidelberg based in Mannheim, with practical experience building leakage aware machine learning pipelines on real sensor data and translating raw device signals into decision support. My six month collaboration with eRay GmbH built recursive forecasting for four water quality indicators with CatBoost multi quantile regression, gate check orchestration, and MICE imputation, the same kind of predictive maintenance mindset that a Preventative Service thesis needs. I combine that with a Bachelor Thesis on clinical ML documented as an IEEE style paper, so I can carry a thesis from literature review to publishable evaluation on my own.
